...
const tc: TestCase = {
  ...
  steps: [
    ...
    { id: 8, desc: '断言成员标签可见', action: 'assert', check: { type: 'text_visible', value: '成员' } },
    { id: 9, desc: '断言成员列表存在', action: 'assert', check: { type: 'element_visible', locator: { by: 'css', value: '.member-list' } } },
  ],
  expect: { result: 'pass' },
};
