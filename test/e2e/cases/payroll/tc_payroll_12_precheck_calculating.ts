...
const tc: TestCase = {
  id: 'TC-PAY-12',
  title: '预检时存在 CALCULATING 状态周期时预检失败',
  module: 'payroll',
  priority: 'P0',
  tags: ['smoke'],
  ...
  steps: [
    ...
    { id: 9, desc: '断言结算中提示', action: 'assert', check: { type: 'toast_contains', value: '结算中' } },
    { id: 10, desc: '断言不能重复结算', action: 'assert', check: { type: 'text_visible', value: '结算中' } },
  ],
  expect: { result: 'pass' },
};
