...
const tc: TestCase = {
  ...
  steps: [
    ...
    { id: 8, desc: '点击发布按钮', action: 'click', locator: { by: 'role', role: 'button', name: '发布' } },
    { id: 9, desc: '确认发布', action: 'click', locator: { by: 'role', role: 'button', name: '确认' } },
    { id: 10, desc: '断言发布成功提示', action: 'assert', check: { type: 'toast_contains', value: '发布成功' } },
    { id: 11, desc: '断言状态变为已发布', action: 'assert', check: { type: 'text_visible', value: '已发布' } },
  ],
  expect: { result: 'pass' },
};
