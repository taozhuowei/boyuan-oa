...
const tc: TestCase = {
  id: 'TC-PAY-11',
  title: '预检时存在 PENDING_REVIEW 异议单时预检失败',
  module: 'payroll',
  priority: 'P0',
  tags: ['smoke'],
  ...
  steps: [
    ...
    { id: 9, desc: '断言预检失败提示', action: 'assert', check: { type: 'toast_contains', value: '异议' } },
    { id: 10, desc: '断言不能结算', action: 'assert', check: { type: 'element_hidden', locator: { by: 'role', role: 'button', name: '结算' } } },
  ],
  expect: { result: 'pass' },
};
