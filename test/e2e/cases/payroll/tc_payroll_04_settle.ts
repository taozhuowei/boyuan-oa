...
const tc: TestCase = {
  ...
  steps: [
    ...
    { id: 9, desc: '确认结算', action: 'click', locator: { by: 'role', role: 'button', name: '确认' } },
    { id: 10, desc: '等待结算完成', action: 'wait', ms: 5000 },
    { id: 11, desc: '断言结算完成提示', action: 'assert', check: { type: 'toast_contains', value: '结算完成' } },
    { id: 12, desc: '断言状态变为已结算', action: 'assert', check: { type: 'text_visible', value: '已结算' } },
  ],
  expect: { result: 'pass' },
};
