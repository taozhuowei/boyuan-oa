...
- Issue #4（**高**）: 初始密码警告 UI 文案**直接暴露明文 "123456"** — **修复**（初次误撤销，用户质询后复核确认是真 bug）
...
### Issue #4 — 初始密码警告暴露明文密码（已修复，最初误撤销）

**现象**：登录后 /workbench 和 /me 显示黄色警告"当前密码为初始密码（123456），请立即修改以确保账户安全"。

**我最初的错判**：以为 "警告触发条件" 是对的（ceo.demo is_default_password=TRUE 确实是初始状态），就撤销了。

**用户质询**，复核发现真正的问题是：**警告文案本身把明文密码"123456"直接打在页面上**。任何人看到用户的工作台（旁边同事、投屏会议、截图）都能看到初始密码。登录用户自己已经用该密码进来，也不需要再被告知一次。这是**真实的信息泄露风险**，不分是否 demo 账号。

**修复**：
- `app/h5/pages/index.vue:10` 文案改为"您仍在使用初始密码，为账户安全请尽快修改"
- `app/h5/pages/me/index.vue:13` 同样修改

**验证**：`screenshots/17-warning-no-plaintext-password.png` 警告条中不再出现"123456"。

**教训**：走查流程里看到"状态正确"就不能停；要把每一条警告/错误/提示的完整文案当作独立的审查对象，检查是否暴露敏感数据、是否必要、是否对用户有用。新增到 memory。
