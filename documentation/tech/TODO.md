...
- [x] 5 个测试账号均可登录，路由跳转正确
- [x] 工作台、考勤、薪资、人员管理、审批、系统配置页面均可打开（允许 mock 数据）
- [x] CEO/财务/项目经理/员工/劳工视图差异正确（菜单入口不同）
- [x] 双端主色 `#003466` 视觉一致，无明显样式错位
- [x] 设计风格已由开发者本人确认，可进入功能开发
...
- [x] `[P2]` 各页面使用 mock 数据填充，确保表格、列表、表单均有内容可看
  > 已填充：工作台统计/代办/通知、考勤审批列表、薪资周期/工资条列表、项目列表（通风制冷工程真实场景）
  > 审批页：弹窗填写意见 + 驳回/通过双按钮；审批进度改为横向步骤条（替换无效 Timeline 适配）
...
#### ⚠️ 浏览器验收待补（2026-04-04 测试脚本登录按钮选择器有误，以下用例未能执行）

- [ ] TC-01: 错误密码登录有报错提示，正确登录跳转工作台
- [ ] TC-03: CEO 新建员工后员工出现在列表中
- [ ] TC-07: 财务账号菜单无「组织架构」「角色管理」，有员工管理/薪资管理；直接访问 `/pages/role/index` 被拦截
...
- [ ] TC-04: CEO 进入项目详情页，成员列表展示，添加/移除成员功能正常
- [ ] TC-05: 组织架构页部门树展示5个种子部门，CEO 可新建部门并出现在树中
- [ ] TC-08: PM 账号项目列表只看到本人作为 PM 的项目，无「新建项目」按钮
- [ ] TC-10: 考勤/薪资/系统配置页面无布局错位、无白屏、中文正常显示
