...
| **M11 测试 & 上线**     | 🔴 0%    | M0–M10               | 覆盖率达标，可部署到生产                   |
| **M12 初始化向导**      | 🔴 0%    | M0–M10（低优先级）   | 向导走通，入口锁定                         |
...
**当前状态（2026-04-08）：**
- ✅ M0-M4, M6, M8 已完成（绿色节点）
- 🟡 M5 薪资 85% — 缺电子签名/确认协议/社保模式/异议更正
- 🔴 M9 通知工作台 0% — **关键阻塞项**
- 🔴 M10 数据生命周期 0% — 依赖 M5 + M9
- 🔴 M11 测试上线 0% — 依赖 M0-M10
- 🔴 M12 初始化向导 0% — 依赖 M0-M10
- ⏸️ M-MP 小程序 — **M11 完成后启动**
...
- [x] `[P1]` 安装 `concurrently` 为 workspace devDependency，用于并行启动多个进程
  > 完成：`app/package.json` 已创建，含 concurrently devDependency 和全套 workspace 脚本（2026-04-09）

- [x] `[P1]` `npm run dev` — 并行启动 H5 前端 + 后端，单命令完成本地开发环境启动
  ```json
...
- [x] `[P1]` PostgreSQL 生产 schema 迁移脚本（Flyway 分模块目录，见 ARCHITECTURE §12.5）
  > 完成：`server/src/main/resources/db/migration/V1__init_schema.sql` + `V2__init_data.sql`，纯 PostgreSQL 语法（2026-04-09）

- [x] `[P1]` `application-prod.yml`（PostgreSQL、文件存储路径、JWT 密钥环境变量）
  > 已存在：使用 ${JWT_SECRET}、${DB_URL} 等环境变量，Flyway 配置 classpath:db/migration
...
- [ ] `[P1]` GitHub Actions CI workflow：push 到任意分支触发后端测试 + 前端测试
  > 检查：`.github/workflows/ci.yml` 存在；push 后 Actions 页显示 `mvn test` 和 `vitest` 均通过；任一失败则 workflow 非零退出

- [ ] `[P1]` GitHub Actions Release workflow：merge to main 触发构建制品（后端 JAR + H5 dist）
  > 检查：`.github/workflows/release.yml` 存在；merge 后 Actions Artifacts 中有 `app.jar` 和 `h5-dist.zip`；JAR 以 `MANIFEST.MF` `Implementation-Version` 记录版本号

- [ ] `[P2]` 版本号注入：`git tag`（如 `v1.2.0`）→ Gradle `archiveVersion` → JAR manifest；同步注入前端 `VITE_APP_VERSION` 环境变量
  > 检查：`java -jar app.jar --version` 或读取 manifest 可得版本号；前端 `import.meta.env.VITE_APP_VERSION` 有值

### 在线更新与回滚任务

- [ ] `[P1]` `tools/deploy/update.sh`：备份旧 JAR（`app-previous.jar`）→ 部署新 JAR → 重启服务 → 健康检查，失败自动调用 rollback.sh
  > 检查：脚本存在且可执行；部署成功后 `/actuator/health` 返回 UP；模拟健康检查失败时服务自动回滚至前一版本

- [ ] `[P1]` `tools/deploy/rollback.sh`：将 `app-previous.jar` 恢复为 `app.jar` → 重启服务 → 健康检查
  > 检查：脚本存在且可执行；执行后服务以旧版本启动；`/actuator/health` 返回 UP

- [ ] `[P1]` `tools/deploy/VERSION`：纯文本文件，记录当前部署版本号；update.sh 成功后自动写入新版本，rollback.sh 回滚后恢复旧版本
  > 检查：文件存在；内容与运行中 JAR 的 manifest 版本一致
