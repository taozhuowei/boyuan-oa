...
- [ ] CI/CD 流水线就绪（push 触发自动测试，merge to main 触发构建制品）
- [ ] 后端 update.sh / rollback.sh 可执行，健康检查失败时自动回滚

### 测试任务

- [x] `[P0]` 后端 Service 层单元测试（算薪引擎、审批流引擎、签名存证）— 见 TEST_DESIGN §2.1
  > 完成：PayrollEngineTest/ApprovalFlowServiceTest/SignatureServiceTest 存在于 server/src/test/java/com/oa/backend/service/；`mvn test` 110 个测试全通过（2026-04-09）
