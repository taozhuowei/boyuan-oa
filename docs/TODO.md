...
- [ ] `[P0]` `TraceIdFilter`：每个 HTTP 请求生成 UUID trace_id，写入 MDC，响应头携带 `X-Trace-Id`（见 ARCHITECTURE §13.3）
  > 验证：`filter/` 目录不存在，需新建 `TraceIdFilter.java`，搜索 `MDC.put("trace_id"...)` 后确认

- [ ] `[P1]` `OperationLogAspect`：`@OperationLogRecord` 注解驱动，自动拦截关键操作写日志（见 ARCHITECTURE §13.5）
  > 验证：`aspect/` 目录不存在，需新建 `OperationLogAspect.java`，含 `@Around("@annotation(OperationLogRecord)")` 切点

- [ ] `[P1]` 将 `OaDataService` 表单/审批内存逻辑迁移到真实 Service + Mapper
  > 验证：`WorkLogController.java` 仍使用 `OaDataService`，需改为调用 `FormService`

- [ ] `[P1]` 审批流定义 CRUD 接口（`GET/PUT /approval/flows/{type}`）
  > 检查：`ApprovalFlowController.java` — 确认 GET 列表、PUT 更新审批流节点配置均存在

### 前端任务

- [ ] `[P1]` 待办中心页（`pages/todo/`）：聚合所有待我审批，按类型/状态筛选
  > 检查：`pages/todo/` 目录存在，调用 `GET /attendance/todo`，支持筛选

- [ ] `[P1]` 审批通过/驳回操作（含驳回原因输入）
  > 检查：搜索 `POST /attendance/{id}/approve` 和 `/reject` 调用；驳回时弹出原因输入框

- [ ] `[P1]` `components/customized/ApprovalTimeline.vue` — 审批历史时间轴组件
  > 检查：文件存在，接受 steps 数组 prop，每步含操作人/时间/结果
...
- [ ] 员工提交请假单，状态 PENDING；PM 初审→APPROVING；CEO 终审→APPROVED
- [ ] 驳回后申请人可重新发起（新单据，历史保留）
- [ ] 无直系领导时，节点1自动转交 CEO
- [ ] PM 发起加班通知，员工可确认/拒绝；CEO 发起的加班通知直接归档
- [ ] 员工发起自补加班申请，走双审流程（直系领导+CEO）
...
- [ ] `[P1]` 历史记录接口（`GET /attendance/history`，支持按项目/时间范围筛选）
  > 检查：`AttendanceController.java` — 搜索 `/history` 路由，支持 PM 按项目筛选

- [ ] `[P1]` 考勤计量单位配置（`GET/POST /config/attendance-unit`，HOUR/HALF_DAY/DAY）
  > 检查：Controller — 搜索 `attendance-unit` 路由，确认读取和更新均存在

- [ ] `[P1]` 加班通知发起接口（`POST /overtime-notifications`）
  > 检查：`OvertimeNotificationController.java` — POST 方法存在；CEO 发起直接 CONFIRMED；PM 发起状态 NOTIFIED

- [ ] `[P1]` 加班通知响应接口（`POST /overtime-notifications/{id}/respond`）
  > 检查：Controller — 搜索 `/respond` 路由；拒绝时触发直系领导审批

- [ ] `[P1]` 自补加班申请接口（`POST /attendance/overtime-self-report`，走双审流程）
  > 检查：`AttendanceController.java` — 搜索 `overtime-self-report`；写入 form_record 并启动两节点审批流
...
- [ ] `[P0]` 考勤页接入真实接口（提交请假、获取历史记录）
  > 检查：`pages/attendance/index.vue` — onMounted 调用 `GET /attendance/records`；提交调用 POST 接口，无 hardcoded mock

- [ ] `[P1]` 加班通知列表（员工/劳工视图）：展示收到的通知，支持确认/拒绝
  > 检查：attendance/index.vue 加班通知 Tab — 调用 `GET /overtime-notifications?recipientId={me}`

- [ ] `[P1]` 加班通知管理页（PM/CEO 视图）：发起通知、查看响应情况
  > 检查：搜索 `POST /overtime-notifications` 调用；通知详情展示响应列表

- [ ] `[P1]` 自补加班申请表单（含附件上传，调用 `POST /attendance/overtime-self-report`）
  > 检查：attendance/index.vue — 自补加班 Tab 存在；提交时携带 attachmentIds
...
**实际代码验证（2026-04-07）**：
- PayrollController.java：❌ 全部使用 OaDataService（内存），listCycles/precheckCycle/settleCycle 均调用内存服务
- payroll/index.vue：❌ MOCK（注释 "Phase 1 使用 mock 数据"，handleAction 显示"功能开发中"）
- 本模块所有后端/前端任务均未开始

### 检查点（全部通过才进入 M6）

- [ ] 窗口期到期后自动锁定（无手动关闭接口），财务可查看各员工数据状态
- [ ] 财务发起结算，2 项强制检查（无 PENDING_REVIEW + 无 CALCULATING）通过后锁定周期
- [ ] 员工端工资单"待确认"，完成电子签名绑定 + PIN 设置后可确认
- [ ] 工资条确认后生成存证 PDF（含签名、意图声明、时间戳水印）
- [ ] 财务发起更正，CEO 审批解锁，重算后 version 递增，历史版本保留

### 后端任务

- [ ] `[P0]` 将 `OaDataService` 薪资内存逻辑迁移到真实 Service + Mapper
  > 检查：`OaDataService.java` — 薪资相关方法已删除；PayrollSlip/PayrollCycle 通过真实 Mapper 写入 DB

- [ ] `[P0]` `PayrollEngine.openWindow()` / `getWindowStatus()`；`PayrollWindowScheduler` 到期自动锁定（无 `closeWindow()` 方法）
  > 检查：`PayrollEngine.java` — 含 openWindow/getWindowStatus；**确认不存在** closeWindow()；`PayrollWindowScheduler.java` — 扫描 `window_status=OPEN` 且 `window_end_date <= NOW()` 的记录

- [ ] `[P0]` 结算前置检查（仅 2 项强制）：① 无 PENDING_REVIEW 状态 PayrollSlip；② 无 CALCULATING 状态 PayrollCycle
  > 检查：`PayrollEngine.java` — settle() 仅检查上述 2 项；读取岗位/等级/LeaveTypeDef/SocialInsuranceItem/PayrollItemDef 计算 PayrollSlip + PayrollSlipItem

- [ ] `[P0]` 正式结算：锁定周期，生成工资条，发布 `PayrollCycleLockedEvent`
  > 检查：`PayrollController.java` — `/cycles/{id}/settle` POST；执行后 `payroll_cycle.locked=true`，生成 payroll_slip 记录
...
- [ ] `[P1]` 薪资页接入真实接口（`GET /payroll/cycles`、`GET /payroll/slips`）
  > 检查：`pages/payroll/index.vue` — onMounted 调用真实接口，无 hardcoded mock

- [ ] `[P1]` 窗口期管理 Tab（财务视图）：展示窗口期剩余时间和各员工数据状态；**无提前关闭按钮**
  > 检查：payroll/index.vue — 窗口期 Tab 存在；**确认不存在"提前关闭"按钮**

- [ ] `[P1]` 预结算发起页：展示 2 项强制检查清单；全通过后"发起结算"按钮激活
  > 检查：payroll 预结算页 — 只展示 2 个强制检查项，无"提交例外申请"入口
...
- [ ] `[P1]` 工资条详情页：工资项明细展示（社保按模式展示"扣款"或"补贴"标签）
  > 检查：payroll 详情页 — 两种社保模式标签差异正确

- [ ] `[P2]` 工资异议发起（`POST /payroll/slips/{id}/dispute`）+ 更正历史版本查看
