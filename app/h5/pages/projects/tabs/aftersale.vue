    ...
    >
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'action'">
          <a-button type="link" size="small" @click="openTicketEdit(record as AfterSaleTicket)">
            编辑
          </a-button>
        </template>
      </template>
    ...
    >
      <a-form layout="vertical" :model="ticketForm">
        <a-row :gutter="16">
          <a-col :span="12">
            <a-form-item label="问题类型" required>
              <a-select v-model:value="ticketForm.typeCode">
                <a-select-option v-for="t in ticketTypes" :key="t.code" :value="t.code">
                  {{ t.name }}
                </a-select-option>
              </a-select>
            </a-form-item>
          ...
          <a-col :span="12">
            <a-form-item label="售后日期" required>
              <a-input v-model:value="ticketForm.incidentDate" placeholder="YYYY-MM-DD" />
            </a-form-item>
          </a-col>
        </a-row>
        <a-form-item label="问题描述" required>
          <a-textarea v-model:value="ticketForm.description" :rows="3" />
        </a-form-item>
        <a-form-item label="客户反馈">
          <a-textarea v-model:value="ticketForm.customerFeedback" :rows="2" />
        </a-form-item>
        <a-form-item label="处理结果">
          <a-textarea v-model:value="ticketForm.resolution" :rows="2" />
        </a-form-item>
...
async function loadTickets() {
  ...
  try {
    tickets.value =
      (await request<AfterSaleTicket[]>({
        url: `/after-sale/tickets?projectId=${props.projectId}`,
      })) ?? []
  } catch {
    tickets.value = []
  } finally {
...
async function submitTicket() {
  if (
    !ticketForm.value.typeCode ||
    !ticketForm.value.incidentDate ||
    !ticketForm.value.description
  ) {
    message.warning('类型/日期/描述必填')
    return
  }
