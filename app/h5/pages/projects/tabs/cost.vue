...
const materialForm      = ref<{
  itemName: string; spec: string; quantity: number | undefined
  unit: string; unitPrice: number | undefined; occurredOn: string; remark: string
}>({
  itemName: '', spec: '', quantity: undefined, unit: '',
  unitPrice: undefined, occurredOn: new Date().toISOString().slice(0, 10), remark: ''
})
...
async function loadMaterialCosts() {
  ...
  try {
    materialCosts.value = await request<MaterialCost[]>({ url: `/projects/${props.projectId}/material-costs` }) ?? []
...
async function submitMaterialCost() {
  ...
  try {
    await request({ url: `/projects/${props.projectId}/material-costs`, method: 'POST', body: materialForm.value })
    message.success('已录入')
...
const insuranceForm    = ref<{
  ...
  insuranceName: '', scope: 'GLOBAL', scopeTargetId: undefined,
  dailyRate: undefined, effectiveDate: new Date().toISOString().slice(0, 10), remark: ''
})
...
async function loadInsurance() {
  ...
  try {
    ...
    insuranceRows.value = await request<InsuranceRow[]>({ url: `/projects/${props.projectId}/insurance/summary` }) ?? []
  } catch { insuranceRows.value = [] } finally { insuranceLoading.value = false }
}
...
async function submitInsurance() {
  ...
  try {
    await request({ url: `/projects/${props.projectId}/insurance`, method: 'POST', body: insuranceForm.value })
    message.success('已新建')
