...
class ApprovalFlowServiceTest {
    ...
    void initFlow_nullSkipCondition_noSkip() {
        ...
        verify(approvalRecordMapper, never()).insert(argThat(record ->
                "SKIP".equals(record.getAction())
        ));
    }
    ...
    void initFlow_invalidSkipConditionJson_noSkip() {
        ...
        ApprovalFlowNode node1 = createNode(1L, TEST_FLOW_DEF_ID, 1, "主管审批", "DESIGNATED", "20", "invalid json");
        when(approvalFlowNodeMapper.findByFlowId(TEST_FLOW_DEF_ID)).thenReturn(Collections.singletonList(node1));
    ...
    void canApprove_directSupervisorApproverType() {
        ...
        ApprovalFlowNode node = createNode(1L, TEST_FLOW_DEF_ID, 1, "主管审批", "DIRECT_SUPERVISOR", null, null);
    ...
    void canApprove_designatedInvalidId_returnsFalse() {
        ...
        ApprovalFlowNode node = createNode(1L, TEST_FLOW_DEF_ID, 1, "指定审批", "DESIGNATED", "invalid_id", null);
        when(approvalFlowNodeMapper.findByFlowIdAndNodeOrder(TEST_FLOW_DEF_ID, 1)).thenReturn(node);
    ...
    void resolveRoleApprover_noEmployeeFound_fallbackToCEO() {
        ...
        ApprovalFlowNode node = createNode(1L, TEST_FLOW_DEF_ID, 1, "角色审批", "ROLE", "nonexistent_role", null);
        when(approvalFlowNodeMapper.findByFlowIdAndNodeOrder(TEST_FLOW_DEF_ID, 1)).thenReturn(node);

        when(employeeMapper.selectOne(any())).thenReturn(null).thenReturn(null); // First for role, second for CEO
    ...
    void advance_writesOperationLog() {
        ...
        ApprovalFlowNode node = createNode(1L, TEST_FLOW_DEF_ID, 1, "审批", "DESIGNATED", String.valueOf(TEST_APPROVER_ID), null);
        when(approvalFlowNodeMapper.findByFlowIdAndNodeOrder(TEST_FLOW_DEF_ID, 1)).thenReturn(node);
        when(approvalFlowNodeMapper.findByFlowId(TEST_FLOW_DEF_ID)).thenReturn(Collections.singletonList(node));
    ...
    void getTodo_success_returnsFormsEmployeeCanApprove() {
        ...
        when(approvalFlowNodeMapper.findByFlowIdAndNodeOrder(TEST_FLOW_DEF_ID, 1))
            .thenReturn(createNode(1L, TEST_FLOW_DEF_ID, 1, "主管审批", "DESIGNATED", String.valueOf(employeeId), null));
        ...
        when(approvalFlowNodeMapper.findByFlowIdAndNodeOrder(TEST_FLOW_DEF_ID + 1, 1))
            .thenReturn(createNode(2L, TEST_FLOW_DEF_ID + 1, 1, "经理审批", "DESIGNATED", String.valueOf(employeeId), null));
    ...
    void resolveApproverId_designatedValidId_canApproveReturnsTrue() {
        ...
        ApprovalFlowNode node = createNode(1L, TEST_FLOW_DEF_ID, 1, "指定审批", "DESIGNATED", String.valueOf(designatedApproverId), null);
        when(approvalFlowNodeMapper.findByFlowIdAndNodeOrder(TEST_FLOW_DEF_ID, 1)).thenReturn(node);
    ...
    void buildFormRecordResponse_withDepartment_returnsDepartmentName() {
        ...
        ApprovalFlowNode node = createNode(1L, TEST_FLOW_DEF_ID, 1, "主管审批", "DESIGNATED", String.valueOf(TEST_APPROVER_ID), null);
        when(approvalFlowNodeMapper.findByFlowIdAndNodeOrder(TEST_FLOW_DEF_ID, 1)).thenReturn(node);
        when(approvalFlowNodeMapper.findByFlowId(TEST_FLOW_DEF_ID)).thenReturn(Collections.singletonList(node));
