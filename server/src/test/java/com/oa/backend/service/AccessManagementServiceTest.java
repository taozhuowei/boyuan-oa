...
/** AccessManagementService 单元测试 覆盖：认证、角色管理、权限检查、演示账号逻辑 */
@ExtendWith(MockitoExtension.class)
@MockitoSettings(strictness = Strictness.LENIENT)
@DisplayName("AccessManagementService - 访问管理服务测试")
class AccessManagementServiceTest {

  @InjectMocks private AccessManagementService service;
  ...
  void authenticate_correctCredentialsReturnsUser() {
    ...
    Optional<AccessManagementService.AuthenticatedUser> result =
        service.authenticate("employee.demo", "123456");
  ...
  void authenticate_wrongPasswordReturnsEmpty() {
    ...
    Optional<AccessManagementService.AuthenticatedUser> result =
        service.authenticate("employee.demo", "wrongpassword");
  ...
  void authenticate_nonExistentUserReturnsEmpty() {
    Optional<AccessManagementService.AuthenticatedUser> result =
        service.authenticate("nonexistent.user", "123456");
  ...
  void listRoles_rolesContainCorrectPermissions() {
    ...
    RoleViewResponse employeeRole =
        roles.stream().filter(r -> r.roleCode().equals("employee")).findFirst().orElseThrow();
    ...
    RoleViewResponse ceoRole =
        roles.stream().filter(r -> r.roleCode().equals("ceo")).findFirst().orElseThrow();
  ...
  void createRole_successfullyCreatesNewRole() {
    RoleUpsertRequest request =
        new RoleUpsertRequest("test_role", "测试角色", "用于测试的角色", 1, List.of("权限1", "权限2"));
  ...
  void createRole_normalizesPermissions() {
    RoleUpsertRequest request =
        new RoleUpsertRequest(
            "test_role", "测试角色", null, null, Arrays.asList("  权限1  ", "", null, "权限2"));
  ...
  void updateRole_successfullyUpdatesExistingRole() {
    RoleUpsertRequest createRequest =
        new RoleUpsertRequest("updatable_role", "原角色名", null, null, List.of("原权限"));
    RoleViewResponse created = service.createRole(createRequest);

    RoleUpsertRequest updateRequest =
        new RoleUpsertRequest("updated_role", "新角色名", "新描述", 0, List.of("新权限1", "新权限2"));
  ...
  void updateRole_nonExistentRoleThrowsException() {
    ...
    IllegalArgumentException ex =
        assertThrows(IllegalArgumentException.class, () -> service.updateRole(99999L, request));
  ...
  void updateRole_nullStatusKeepsOriginal() {
    ...
    RoleUpsertRequest updateRequest =
        new RoleUpsertRequest("status_test", "状态测试", null, null, null);
    RoleViewResponse updated = service.updateRole(created.id(), updateRequest);
  ...
  void deleteRole_systemRoleThrowsException() {
    IllegalStateException ex =
        assertThrows(IllegalStateException.class, () -> service.deleteRole(1L));
  ...
  void deleteRole_nonExistentRoleThrowsException() {
    IllegalArgumentException ex =
        assertThrows(IllegalArgumentException.class, () -> service.deleteRole(99999L));
    assertTrue(ex.getMessage().contains("不存在"));
  }
  ...
  void buildProfile_successfullyBuildsProfile() {
    AccessManagementService.AuthenticatedUser profile =
        service.buildProfile("test.user", "测试用户", "employee");
  ...
  void buildProfile_normalizesRoleCode() {
    AccessManagementService.AuthenticatedUser profile =
        service.buildProfile("test.user", "测试用户", "Employee-Role");
  ...
  void buildProfile_nonExistentRoleUsesDefault() {
    AccessManagementService.AuthenticatedUser profile =
        service.buildProfile("test.user", "测试用户", "nonexistent_role");
  ...
  void seededDemoAccount_financeDemoConfiguration() {
    ...
    Optional<AccessManagementService.AuthenticatedUser> result =
        service.authenticate("finance.demo", "123456");
  ...
  void seededDemoAccount_pmDemoConfiguration() {
    ...
    Optional<AccessManagementService.AuthenticatedUser> result =
        service.authenticate("pm.demo", "123456");
  ...
  void seededDemoAccount_ceoDemoConfiguration() {
    ...
    Optional<AccessManagementService.AuthenticatedUser> result =
        service.authenticate("ceo.demo", "123456");
  ...
  void seededDemoAccount_workerDemoConfiguration() {
    ...
    Optional<AccessManagementService.AuthenticatedUser> result =
        service.authenticate("worker.demo", "123456");
  ...
  void seededSystemRole_financePermissions() {
    ...
    RoleViewResponse finance =
        roles.stream().filter(r -> r.roleCode().equals("finance")).findFirst().orElseThrow();
  ...
  void seededSystemRole_workerPermissions() {
    ...
    RoleViewResponse worker =
        roles.stream().filter(r -> r.roleCode().equals("worker")).findFirst().orElseThrow();
