...
class FormServiceTest {
  ...
  void getFormTypeName_variousTypes_shouldReturnCorrectNames() {
    ...
    List<FormRecord> records =
        Arrays.asList(leaveRecord, overtimeRecord, injuryRecord, logRecord, unknownRecord);

    when(formRecordMapper.selectList(any())).thenReturn(records);

    // When
    List<FormRecordResponse> result =
        formService.getHistory(employeeId, roleCode, Collections.emptyList());
  ...
  void getApprovalHistory_shouldBuildHistoryCorrectly() {
    ...
    when(employeeMapper.selectById(requesterId)).thenReturn(requester);
    when(approvalFlowService.canApprove(any(), any())).thenReturn(false);
    when(approvalRecordMapper.findByFormId(formId))
        .thenReturn(Collections.singletonList(approvalRecord));
