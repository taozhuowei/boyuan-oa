 ...
 * 认证控制器测试类
 *
 * <p>验证 AuthController、RoleController、HealthController 的接口行为， 包括开发环境登录、密码登录、JWT 令牌验证等功能。
...
class AuthControllerTest {
  ...
  @MockBean private AccessManagementService accessManagementService;

  @MockBean private com.oa.backend.service.EmployeeService employeeService;

  @MockBean private com.oa.backend.security.ResetCodeStore resetCodeStore;

  @MockBean private com.oa.backend.service.AuthDataService authDataService;
   ...
   * 测试开发环境登录接口
   *
   * <p>验证：POST /auth/dev-login 端点在提供有效用户信息时能正确返回 JWT 令牌 断言：HTTP 200；token
   * 非空；mode=DEV_LOGIN；role=ceo
   */
  @Test
  void shouldReturnJwtForDevelopmentLogin() throws Exception {
    given(accessManagementService.buildProfile("ceo.demo", "测试 CEO", "ceo"))
        .willReturn(
            new AccessManagementService.AuthenticatedUser(
                "ceo.demo", "测试 CEO", "ceo", "首席经营者", "总经办", "普通员工"));
   ...
   * 测试密码登录接口
   *
   * <p>验证：POST /auth/login 端点在提供正确用户名密码时能正确返回 JWT 令牌 断言：HTTP 200；mode=PASSWORD_LOGIN；role=finance
   */
  @Test
  void shouldReturnJwtForPasswordLogin() throws Exception {
    ...
    given(employeeService.authenticate("finance.demo", "123456")).willReturn(Optional.of(employee));
    given(
            authDataService.resolveRoleName(
                org.mockito.Mockito.eq("finance"),
   ...
   * 测试无令牌访问受保护端点
   *
   * <p>验证：未提供 Authorization 头时，访问受保护资源应被拒绝 断言：HTTP 401
   */
  @Test
