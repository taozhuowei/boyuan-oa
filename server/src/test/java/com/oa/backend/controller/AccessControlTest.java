 ...
 *
 * 验证基于角色的访问控制规则，确保：
 * 1. 未认证用户无法访问受保护端点（返回401）
 * 2. 员工角色无法访问财务专属端点（返回403）
 * 3. 财务角色可以访问财务端点（返回2xx或404，但不能是403）
 * 4. 劳工角色可以访问施工日志端点
 * 5. 非劳工角色无法访问劳工专属施工日志端点
 *
 * 使用 @SpringBootTest + @AutoConfigureMockMvc 加载完整应用上下文，
 * 包括安全配置、JWT过滤器等，确保测试接近真实运行环境。
 */
@SpringBootTest
...
class AccessControlTest {
    ...
    /**
     * 测试：FINANCE角色可以访问工资周期列表（不应返回403）
     * 实际状态可能是200（有数据）或404（无数据），但绝不能是403
     */
    @Test
    ...
    /**
     * 测试：WORKER角色可以提交施工日志（不应返回403）
     * 注意：可能返回400（数据验证失败）或404（员工不存在），但绝不能是403
     */
    @Test
    ...
    void finance_constructionLog_returns403() throws Exception {
        mockMvc.perform(post("/logs/construction-logs")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {
                                    "formData": {
    ...
    void employee_injuryReport_returns403() throws Exception {
        mockMvc.perform(post("/logs/injury")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {
                                    "formData": {
    ...
    void finance_recallLog_returns403() throws Exception {
        mockMvc.perform(post("/logs/construction-logs/1/recall")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {
                                    "reason": "Test recall"
