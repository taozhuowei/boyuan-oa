 ...
 *
 * <p>运行方式：mvn test（Spring Boot Test + H2 内存库 + data.sql 种子数据）
 * 报告输出：target/surefire-reports/（XML）、target/site/jacoco/（覆盖率 HTML）
 *
 * <p>种子账号（data.sql）： employee.demo / finance.demo / pm.demo / ceo.demo / worker.demo — 密码均为 123456
 */
@SpringBootTest
...
class OaApiIntegrationTest {
  ...
  class M1Employee {
    ...
    void listEmployees_authorizedRoles_returns200() throws Exception {
      for (String token : new String[] {financeToken, pmToken, ceoToken}) {
        mockMvc
            .perform(get("/employees").header("Authorization", "Bearer " + token))
            .andExpect(status().isOk())
    ...
    void getEmployee_notFound_returns404() throws Exception {
      mockMvc
          .perform(get("/employees/9999").header("Authorization", "Bearer " + ceoToken))
    ...
    void listEmployees_keyword_filtersResults() throws Exception {
      mockMvc
          .perform(
              get("/employees").param("keyword", "李").header("Authorization", "Bearer " + ceoToken))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.content").isArray());
    }
  }
  ...
  class M2Position {
    ...
    void listPositions_returns200() throws Exception {
      mockMvc
          .perform(get("/positions").header("Authorization", "Bearer " + financeToken))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$").isArray());
    }
  ...
  class M2Project {
    ...
    void listProjects_returns200WithData() throws Exception {
      mockMvc
          .perform(get("/projects").header("Authorization", "Bearer " + pmToken))
          .andExpect(status().isOk())
    ...
    void getProject_existingId_returnsProject() throws Exception {
      mockMvc
          .perform(get("/projects/1").header("Authorization", "Bearer " + ceoToken))
          .andExpect(status().isOk())
  ...
  class ImplementedModules {
    ...
    void notifications_returns200() throws Exception {
      mockMvc
          .perform(get("/notifications").header("Authorization", "Bearer " + employeeToken))
          .andExpect(status().isOk());
    }
    ...
    void cleanup_tasks_returns200() throws Exception {
      mockMvc
          .perform(get("/cleanup-tasks").header("Authorization", "Bearer " + ceoToken))
          .andExpect(status().isOk());
    }
  ...
  class M5Payroll {
    ...
    void finance_fullSettlementFlow() throws Exception {
      ...
      int slipCount = objectMapper.readTree(slipsResult.getResponse().getContentAsString()).size();
    ...
    void employee_viewAndConfirmSlip() throws Exception {
      ...
      long slipId =
          objectMapper
              .readTree(slipsResult.getResponse().getContentAsString())
              .get(0)
              .get("id")
              .asLong();
      ...
      mockMvc
          .perform(
              get("/payroll/slips/" + slipId).header("Authorization", "Bearer " + employeeToken))
          .andExpect(status().isOk())
    ...
    void employee_disputeSlip() throws Exception {
      ...
      mockMvc
          .perform(
              post("/payroll/slips/" + slipId + "/dispute")
                  .header("Authorization", "Bearer " + employeeToken)
                  .contentType(MediaType.APPLICATION_JSON)
    ...
    void employee_cannotViewOthersSlip() throws Exception {
      ...
      if (workerSlipId != null) {
        mockMvc
            .perform(
                get("/payroll/slips/" + workerSlipId)
                    .header("Authorization", "Bearer " + employeeToken))
            .andExpect(status().isForbidden());
      }
    }
  ...
  class M6Project {
    ...
    void pm_createMilestone() throws Exception {
      ...
      MvcResult listResult =
          mockMvc
              .perform(get("/projects?size=1").header("Authorization", "Bearer " + ceoToken))
              .andExpect(status().isOk())
              .andReturn();
      long projectId =
          objectMapper
    ...
    void pm_createSummary() throws Exception {
      ...
      mockMvc
          .perform(
              post("/projects/" + projectId + "/construction-summary")
                  .header("Authorization", "Bearer " + pmToken)
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(
                      "{\"periodStart\":\"2026-03-01\",\"periodEnd\":\"2026-03-31\",\"pmNote\":\"Test summary\"}"))
          .andExpect(status().isCreated())
