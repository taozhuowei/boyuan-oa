...
public interface PayrollSlipMapper extends BaseMapper<PayrollSlip> {
     ...
     * 查找指定员工的所有工资条
     */
    @Select("SELECT * FROM payroll_slip WHERE employee_id = #{employeeId} AND deleted = 0 ORDER BY created_at DESC")
    List<PayrollSlip> findByEmployeeId(Long employeeId);

    /**
     * 查找指定周期中特定状态的工资条数量
     */
    @Select("SELECT COUNT(*) FROM payroll_slip WHERE cycle_id = #{cycleId} AND status = #{status} AND deleted = 0")
