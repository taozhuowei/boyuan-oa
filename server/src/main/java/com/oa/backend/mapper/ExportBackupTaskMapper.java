 ...
 *
 * <p>提供导出备份任务的基础 CRUD 操作以及按发起人、状态查询功能。
 *
...
public interface ExportBackupTaskMapper extends BaseMapper<ExportBackupTask> {
   ...
   */
  @Select(
      "SELECT * FROM export_backup_task WHERE initiator_id = #{initiatorId} AND deleted = 0 ORDER BY created_at DESC")
   ...
   */
  @Select(
      "SELECT * FROM export_backup_task WHERE download_token = #{downloadToken} AND deleted = 0")
   ...
   */
  @Select(
      "SELECT * FROM export_backup_task WHERE status = #{status} AND deleted = 0 ORDER BY created_at DESC")
