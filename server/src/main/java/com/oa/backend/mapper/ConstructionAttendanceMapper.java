...
public interface ConstructionAttendanceMapper extends BaseMapper<ConstructionAttendance> {
  ...
  /** 项目内每个员工在指定区间的去重出勤天数（同一 employee+date 计 1 天）。 返回 [{employeeId, days}, ...] */
  @Select(
      "SELECT employee_id AS employeeId, COUNT(DISTINCT attendance_date) AS days "
          + "FROM construction_attendance "
          + "WHERE project_id = #{projectId} AND deleted = 0 "
          + "AND attendance_date BETWEEN #{startDate} AND #{endDate} "
          + "GROUP BY employee_id")
  List<Map<String, Object>> aggregatePerEmployee(
