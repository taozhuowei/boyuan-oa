...
public interface OvertimeNotificationMapper extends BaseMapper<OvertimeNotification> {
     ...
     * 查询指定项目的加班通知（按创建时间倒序）
     */
    @Select("SELECT * FROM overtime_notification WHERE project_id = #{projectId} AND deleted = 0 ORDER BY created_at DESC")
    List<OvertimeNotification> findByProjectId(Long projectId);

    /**
     * 查询发起人的所有通知
     */
    @Select("SELECT * FROM overtime_notification WHERE initiator_id = #{initiatorId} AND deleted = 0 ORDER BY created_at DESC")
