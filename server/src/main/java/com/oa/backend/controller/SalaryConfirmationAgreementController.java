...
/**
 * 工资确认协议控制器
 * 职责：管理员工确认工资条前需同意的协议版本（Finance 上传，全员查看）。
 *
 * 数据流：
 * - 上传：Finance 上传协议版本号和内容，系统自动将该版本设为生效，其他版本设为失效
 * - 查询：所有认证用户可获取当前生效的协议
 *
 * 路由概览：
 * - POST /salary-confirmation-agreement         上传新协议版本（FINANCE only）
 * - GET  /salary-confirmation-agreement/current 获取当前生效协议（所有认证用户）
 */
@RestController
...
public class SalaryConfirmationAgreementController {
     ...
     *
     * 数据流：
     * 1. 校验请求参数（version, content 必填）
     * 2. 获取当前登录用户 ID 作为上传人
     * 3. 将所有现有协议的 is_active 设为 false
     * 4. 插入新协议记录，设置 is_active=true
     *
    ...
    public ResponseEntity<?> uploadAgreement(
            @RequestBody UploadAgreementRequest request,
     ...
     * 权限：所有认证用户
     *
     * 数据流：查询 salary_confirmation_agreement 表中 is_active=true 的最新记录
     *
