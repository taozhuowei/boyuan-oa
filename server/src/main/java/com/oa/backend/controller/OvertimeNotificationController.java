 ...
 * 加班通知控制器
 *
 * <p>加班通知（overtime_notification）是 PM/CEO 直接录入的通知流程（非审批流）。 员工收到通知后可确认接受或拒绝；拒绝需填写原因并由直系领导审批。 CEO
 * 发起的通知员工不可拒绝（直接归档）。
 *
 * <p>Routes: POST /overtime-notifications — PM/CEO 发起加班通知 GET /overtime-notifications —
 * 员工查看收到的通知（按项目成员过滤） GET /overtime-notifications/initiated — PM/CEO 查看自己发起的通知 POST
 * /overtime-notifications/{id}/respond — 员工确认/拒绝通知
 */
@RestController
...
public class OvertimeNotificationController {
  ...
  /** 发起加班通知 权限：项目经理、CEO CEO 发起时通知状态直接为 ARCHIVED（员工无需响应） */
  @PostMapping
  @PreAuthorize("hasAnyRole('PROJECT_MANAGER','CEO')")
  public ResponseEntity<OvertimeNotification> create(
      @RequestBody CreateNotificationRequest req, Authentication authentication) {
  ...
  /** 员工/劳工查看收到的加班通知（通过项目成员关系过滤） 权限：所有登录用户 */
  @GetMapping
  public ResponseEntity<List<NotificationWithResponse>> list(Authentication authentication) {
    ...
    List<NotificationWithResponse> result =
        all.stream()
            .map(
                n -> {
                  OvertimeResponse resp =
                      overtimeNotificationService.findResponse(n.getId(), employee.getId());
  ...
  /** PM/CEO 查看自己发起的通知 权限：项目经理、CEO */
  @GetMapping("/initiated")
  @PreAuthorize("hasAnyRole('PROJECT_MANAGER','CEO')")
  public ResponseEntity<List<NotificationWithResponses>> listInitiated(
      Authentication authentication) {
    Employee initiator = overtimeNotificationService.resolveEmployee(authentication);
    if (initiator == null) return ResponseEntity.badRequest().build();

    List<OvertimeNotification> notifications =
        overtimeNotificationService.listNotificationsByInitiator(initiator.getId());
    List<NotificationWithResponses> result =
        notifications.stream()
            .map(
                n -> {
                  List<OvertimeResponse> responses =
                      overtimeNotificationService.listResponsesByNotification(n.getId());
  ...
  /** 员工响应加班通知（确认或拒绝） 权限：所有登录用户 CEO 发起的通知（ARCHIVED）不可拒绝 */
  @PostMapping("/{id}/respond")
  public ResponseEntity<OvertimeResponse> respond(
      @PathVariable Long id, @RequestBody RespondRequest req, Authentication authentication) {
  ...
  public record RespondRequest(boolean accepted, String rejectReason) {}
  ...
  public record NotificationWithResponses(
      OvertimeNotification notification, List<OvertimeResponse> responses) {}
}
