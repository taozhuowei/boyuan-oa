...
/** 工伤理赔控制器（财务专用） 职责：工伤申报审批通过后，财务录入理赔金额并关联薪资周期。 数据来源：injury_claim 表；关联 form_record.id。 */
@RestController
...
public class InjuryClaimController {
  ...
  /** 录入工伤理赔（财务专用） 收到申请单 formRecordId + 员工信息 + 理赔金额，写入 injury_claim 表，状态置 SETTLED */
  @PostMapping
  @PreAuthorize("hasRole('FINANCE')")
  public ResponseEntity<InjuryClaim> create(
      @Valid @RequestBody InjuryClaimRequest req, Authentication auth) {
    // Resolve employeeId: use request value if provided, otherwise derive from form record
    // submitter via service layer (keeps Mapper access out of controller per architecture rule)
    Long employeeId = req.employeeId();
    if (employeeId == null) {
      employeeId = injuryService.resolveEmployeeIdFromFormRecord(req.formRecordId());
    }
