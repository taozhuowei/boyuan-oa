...
/**
 * 工资项定义控制器 职责：管理系统内置与自定义工资项定义（Finance 可操作自定义项）。 PayrollItemDefMapper 已迁移至 PayrollItemDefService。
 *
 * <p>数据流： - 读取：所有认证用户可查询启用的工资项定义列表 - 写入：Finance 角色可创建、更新、软删除自定义项（isSystem=false） -
 * 系统内置项（isSystem=true）受保护，不可修改 code/type，不可删除
 *
 * <p>路由概览： - GET /payroll/item-defs 查询所有启用的工资项定义（所有认证用户） - POST /payroll/item-defs
 * 创建自定义工资项定义（FINANCE only，isSystem=false） - PUT /payroll/item-defs/{id} 更新工资项定义（FINANCE
 * only，不能修改系统项的 code/type） - DELETE /payroll/item-defs/{id} 软删除工资项定义（FINANCE only，不能删除系统项）
 */
@RestController
...
public class PayrollItemDefController {
  ...
  /**
   * 查询所有启用的工资项定义，按显示顺序升序返回。 权限：所有认证用户
   *
   * @return 启用的工资项定义列表
   */
  @GetMapping
  ...
  /**
   * 创建自定义工资项定义。 权限：FINANCE only 约束：isSystem 强制设为 false（只允许创建自定义项）
   *
  ...
  public ResponseEntity<?> createDef(@RequestBody CreateDefRequest request) {
    ...
    PayrollItemDef def =
        itemDefService.create(
            request.code(), request.name(), request.type(), request.displayOrder());
    return ResponseEntity.ok(def);
  }

  /**
   * 更新工资项定义。 权限：FINANCE only 约束：不能修改系统内置项（isSystem=true）的 code 和 type
   *
   * @param id 工资项定义 ID
  ...
  public ResponseEntity<?> updateDef(@PathVariable Long id, @RequestBody UpdateDefRequest request) {
    ...
    if (Boolean.TRUE.equals(def.getIsSystem())) {
      return ResponseEntity.badRequest().body(Map.of("message", "系统内置项不能修改（code 和 type 受保护）"));
    }
    PayrollItemDef updated =
        itemDefService.update(id, request.name(), request.displayOrder(), request.isEnabled());
    return ResponseEntity.ok(updated);
  }

  /**
   * 软删除工资项定义。 权限：FINANCE only 约束：不能删除系统内置项（isSystem=true）
   *
   * @param id 工资项定义 ID
   * @return 删除结果
   */
  ...
  public ResponseEntity<?> deleteDef(@PathVariable Long id) {
    ...
    // 系统内置项保护：不能删除
    if (Boolean.TRUE.equals(def.getIsSystem())) {
      return ResponseEntity.badRequest().body(Map.of("message", "系统内置项不能删除"));
    }
