...
public class BackupController {
   ...
   * 获取当前用户的导出任务列表。
   *
   * <p>返回当前登录用户发起的所有导出备份任务，按创建时间降序排列。
   *
   ...
   *
   * <p>接受起止日期参数（startDate/endDate），创建状态为 PENDING 的导出任务。 日期参数不落库，任务具体执行由后台进程负责。
   *
  ...
  public ResponseEntity<?> createExportTask(
      @RequestBody ExportTaskCreateRequest req, Authentication authentication) {
    Long initiatorId = SecurityUtils.getCurrentEmployeeId(authentication);
    if (initiatorId == null) {
      return ResponseEntity.status(403).body(Map.of("message", "无法识别当前用户"));
    }
