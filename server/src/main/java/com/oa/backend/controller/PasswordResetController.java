 ...
 * 密码重置控制器
 * 负责处理忘记密码场景的完整三步流程：
 *   1. 发送验证码到手机号 (/auth/send-reset-code)
 *   2. 验证验证码并签发 resetToken (/auth/verify-reset-code)
 *   3. 凭 resetToken 提交新密码完成重置 (/auth/reset-password)
 * 所有端点均无需登录认证，通过 resetToken 机制保证操作安全性。
