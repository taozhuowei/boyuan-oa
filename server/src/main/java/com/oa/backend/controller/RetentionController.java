 ...
 * <p>
 * 所有管理接口（策略查询、提醒列表、操作提醒）仅限 CEO 角色访问。
 * 下载接口需要认证，验证下载令牌有效性。
 *
...
public class RetentionController {
     ...
     * 获取数据保留策略列表。
     * <p>
     * 返回所有未删除的保留策略，包括数据类型、保留年限和提前警告天数。
     *
     * @return 保留策略列表
     */
    @GetMapping("/policies")
     ...
     * 获取到期提醒列表。
     * <p>
     * 返回所有状态为 PENDING 的提醒，按预计删除日期升序排列。
     *
     * @return 到期提醒列表
     */
     ...
     * 导出并删除过期数据。
     * <p>
     * 异步执行导出操作，导出完成后删除过期数据记录。
     * 返回创建的任务 ID，可用于查询任务状态。
     *
     * @param id             提醒 ID
