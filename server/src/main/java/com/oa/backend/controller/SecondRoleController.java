...
/**
 * 第二角色（售后 / 物资管理 / 工长）分配 Controller。 业务逻辑全部委托 {@link SecondRoleService}。
 *
 * <p>Routes: GET /second-roles/defs 定义列表（认证用户） GET /second-roles?employeeId=&projectId= 查询有效分配 POST
 * /second-roles 分配（PM/CEO） DELETE /second-roles/{id} 撤销（PM/CEO）
 */
@RestController
...
public class SecondRoleController {
  ...
  @com.oa.backend.annotation.OperationLogRecord(
      action = "SECOND_ROLE_ASSIGN",
      targetType = "SECOND_ROLE")
  public ResponseEntity<?> assign(@RequestBody AssignRequest req, Authentication auth) {
    SecondRoleService.AssignResult result =
        secondRoleService.assign(
    ...
    if (result instanceof SecondRoleService.AssignResult.Ok ok) {
      ...
      SecondRoleService.AssignResult.Invalid invalid =
          (SecondRoleService.AssignResult.Invalid) result;
