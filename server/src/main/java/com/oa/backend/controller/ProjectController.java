...
/**
 * 项目基础 CRUD 控制器。 负责：创建、查询、更新、删除、状态变更、配置修改。 成员管理见 ProjectMemberController，里程碑/进度见
...
public class ProjectController {
  ...
  /** 从 Authorization 头解析当前用户 ID。 用于 PROJECT_MANAGER 列表过滤。 */
  ...
  /** 从 Authorization 头解析当前用户角色。 用于 PROJECT_MANAGER 列表过滤。 */
  ...
  /**
   * 构建 ProjectResponse。成员信息通过 ProjectService.buildMemberInfos() 获取， 避免 controller 直接依赖
   * EmployeeMapper。
   *
  ...
  /** 获取项目列表（分页）。 PROJECT_MANAGER 自动只看自己作为 PM 的项目。 权限：员工、财务、项目经理、CEO、劳工 */
  @GetMapping
  ...
  /** 获取项目详情（含成员列表）。 权限：员工、财务、项目经理、CEO、劳工 */
  @GetMapping("/{id}")
  @PreAuthorize("hasAnyRole('EMPLOYEE','FINANCE','PROJECT_MANAGER','CEO','WORKER')")
  ...
  /** 更新项目基本信息。 权限：CEO only */
  @PutMapping("/{id}")
  @PreAuthorize("hasRole('CEO')")
  public ResponseEntity<ProjectResponse> updateProject(
      @PathVariable Long id, @RequestBody ProjectUpdateRequest request) {
    Project project = projectService.update(id, request);
    return ResponseEntity.ok(buildProjectResponse(project, true));
  }

  /** 更新项目状态（关闭或重开）。 权限：CEO only */
  @PatchMapping("/{id}/status")
  @PreAuthorize("hasRole('CEO')")
  public ResponseEntity<ProjectResponse> updateProjectStatus(
      @PathVariable Long id, @RequestBody ProjectStatusRequest request) {
  ...
  /** 修改项目配置（施工日志周期、合同号、客户名、描述）。 CEO 直接修改，无需审批。 权限：CEO only */
  @PatchMapping("/{id}/config")
  @PreAuthorize("hasRole('CEO')")
  public ResponseEntity<?> updateProjectConfig(
      @PathVariable Long id, @RequestBody ProjectConfigRequest request) {
    Project project = projectService.getById(id);
    if (project == null) {
      return ResponseEntity.notFound().build();
    }
