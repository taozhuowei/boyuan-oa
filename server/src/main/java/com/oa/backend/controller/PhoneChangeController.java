...
/**
 * 手机号变更控制器
 * 负责处理已登录用户修改绑定手机号的完整四步流程：
 *   1. 发送验证码到当前手机号，验证旧号码所有权 (/auth/phone-change/send-current-code)
 *   2. 验证当前手机号验证码，签发 changeToken (/auth/phone-change/verify-current-code)
 *   3. 发送验证码到新手机号，验证新号码持有 (/auth/phone-change/send-new-code)
 *   4. 提交新号码和验证码，完成绑定变更 (/auth/phone-change/confirm)
 * 所有端点均要求登录认证（@PreAuthorize("isAuthenticated()")）。
 * 业务状态（验证码、changeToken）由 PhoneChangeService 持有，本类仅做 HTTP 分发与参数校验。
 * 从 AuthController 拆分，遵守单一职责原则。
 */
@Slf4j
@RestController
@RequestMapping("/auth")
