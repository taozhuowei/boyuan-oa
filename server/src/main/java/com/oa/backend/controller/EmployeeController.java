...
public class EmployeeController {
  ...
  /** 获取员工列表（分页） 权限：CEO、HR、财务、项目经理、部门主管 */
  @GetMapping
  @PreAuthorize("hasAnyRole('CEO','HR','FINANCE','PROJECT_MANAGER','DEPARTMENT_MANAGER')")
  public ResponseEntity<Map<String, Object>> listEmployees(
    ...
    result.put(
        "content",
        employeePage.getRecords().stream()
            .map(e -> toResponse(e, currentEmployeeId, authentication))
            .toList());
  ...
  /** 创建员工 权限：CEO、HR（DESIGN.md §5.3 HR 负责人员信息录入） */
  @PostMapping
  ...
  /** 更新员工 权限：CEO、HR（DESIGN.md §5.3 HR 负责人员信息录入与维护） */
  @PutMapping("/{id}")
  @PreAuthorize("hasAnyRole('CEO','HR')")
  ...
  /** 职责：重置员工密码为初始密码 请求含义：将指定员工的密码重置为默认初始密码 123456 响应含义：无内容返回（204 No Content） 权限期望：仅 CEO 可操作 */
  ...
  /**
   * 将 Employee 实体转换为 EmployeeResponse DTO（按调用者身份脱敏）。 单记录端点入口（getEmployee、createEmployee
   * 等）；每次调用会解析一次 currentEmployeeId， 不在列表场景下使用，因此单次 DB 查询可接受。
   *
   * <p>脱敏规则： - idCardNo：仅本人、CEO、HR 可见；其他角色返回 null - isDefaultPassword：仅本人可见；查看他人时返回
   * null，避免向攻击者暴露弱默认密码状态
   *
   * <p>保留字段结构（record 不变），仅将敏感字段置 null 以维持向后兼容。
   */
  ...
  /**
   * 列表端点专用重载，接受预计算的 currentEmployeeId 以消除 N+1 查询。 由 listEmployees 在流处理前一次性解析
   * currentEmployeeId，然后传入此方法。
   */
  private EmployeeResponse toResponse(
      Employee employee, Long currentEmployeeId, Authentication authentication) {
    ...
    // 紧急联系人列表（失败兜底为空列表）
    List<EmployeeResponse.EmergencyContact> contacts =
        employeeService.findEmergencyContacts(employee.getId());
