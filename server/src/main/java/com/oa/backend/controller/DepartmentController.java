...
public class DepartmentController {
  ...
  private List<DepartmentResponse> buildDepartmentTree(
      List<Department> departments, Map<Long, Integer> employeeCountMap) {

    // 转换为DTO
    List<DepartmentResponse> allNodes =
        departments.stream()
            .map(
                dept ->
    ...
    Map<Long, List<DepartmentResponse>> parentMap =
        allNodes.stream()
            .collect(Collectors.groupingBy(d -> d.parentId() != null ? d.parentId() : -1L));
  ...
  private List<DepartmentResponse> buildTreeRecursive(
      Long parentId, Map<Long, List<DepartmentResponse>> parentMap) {
    ...
    return children.stream()
        ...
        .sorted(
            Comparator.comparing(DepartmentResponse::sort).thenComparing(DepartmentResponse::id))
  ...
  /** 删除部门 权限：CEO only 逻辑：检查是否有直属员工或子部门 */
  @DeleteMapping("/{id}")
  @PreAuthorize("hasRole('CEO')")
