...
/**
 * 补贴项（allowance_def）及三级覆盖配置（allowance_config）控制器。 底层 Mapper 操作均委托给 AllowanceService。
 *
 * <p>读：allowance_def 及配置均属于薪资敏感数据，仅 CEO/HR/FINANCE 可查。 写：HR / CEO。
 *
 * <p>Routes: GET /allowances 列表（CEO/HR/FINANCE） POST /allowances 创建 PUT /allowances/{id} 更新 DELETE
 * /allowances/{id} 软删除（系统项不可删） GET /allowances/{id}/configs 列出三级覆盖配置 PUT /allowances/{id}/configs
 * 批量覆盖保存（整张表覆盖写入）
 */
@RestController
...
public class AllowanceController {
  ...
  public ResponseEntity<?> update(@PathVariable Long id, @RequestBody DefRequest req) {
    ...
    AllowanceDef updated =
        allowanceService.updateDef(
  ...
  /** 批量覆盖保存该补贴项的所有三级配置（整表替换）。 请求体为当前页保存的全部配置项；服务端会删除旧配置再插入新配置，保持一致性。 */
  ...
  public ResponseEntity<?> saveConfigs(@PathVariable Long id, @RequestBody List<ConfigItem> items) {
    ...
    for (ConfigItem it : items) {
      if (it.scope() == null || !List.of("GLOBAL", "POSITION", "EMPLOYEE").contains(it.scope())) {
        return ResponseEntity.badRequest()
            .body(Map.of("message", "scope 必须为 GLOBAL/POSITION/EMPLOYEE"));
      }
      ...
      if (!"GLOBAL".equals(it.scope()) && it.scopeTargetId() == null) {
        return ResponseEntity.badRequest()
            .body(Map.of("message", "POSITION/EMPLOYEE 必须指定 scopeTargetId"));
      }
    ...
    List<AllowanceService.ConfigItem> serviceItems =
        items.stream()
  ...
  public record DefRequest(
      String code, String name, String description, Integer displayOrder, Boolean isEnabled) {}
