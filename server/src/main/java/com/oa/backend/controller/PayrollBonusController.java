...
/**
 * 周期临时补贴/奖金 Controller。 财务在薪资周期窗口期内录入；结算后锁定。
 *
 * <p>Routes: GET /payroll/cycles/{cycleId}/bonuses 列出周期全部临时补贴（财务/CEO） POST
 * /payroll/cycles/{cycleId}/bonuses 创建（财务） DELETE /payroll/bonuses/{id} 软删除（财务，仅未结算可删） GET
 * /payroll/bonus-approval-config 获取审批开关（认证用户） PUT /payroll/bonus-approval-config 设置审批开关（CEO）
 */
@RestController
...
public class PayrollBonusController {
  ...
  public ResponseEntity<?> create(
      @PathVariable Long cycleId, @RequestBody CreateBonusRequest req, Authentication auth) {
  ...
  public ResponseEntity<Map<String, Boolean>> setApprovalConfig(
  ...
  public record CreateBonusRequest(
      Long employeeId, String name, BigDecimal amount, String type, String remark) {}
