...
public class AttendanceController {
  ...
  /** 全量历史记录（PM/CEO 视图，支持按时间范围筛选） 权限：项目经理、CEO */
  @GetMapping("/history")
  ...
  /** 自补加班申请（补申报历史加班，走审批流） 与 POST /attendance/overtime 的区别：此接口明确表示为补申报，进入 OVERTIME 审批流 权限：员工、劳工 */
    ...
    return new FormConfigResponse(
        ...
        Arrays.asList(
            new FormConfigResponse.FormField(
                ...
                true,
                null,
                Arrays.asList("年假", "事假", "病假", "婚假", "产假"),
                "请选择请假类型",
                1),
            new FormConfigResponse.FormField(
                "startDate", "开始日期", "DATE", true, null, null, "请选择开始日期", 2),
            new FormConfigResponse.FormField(
                "endDate", "结束日期", "DATE", true, null, null, "请选择结束日期", 3),
            new FormConfigResponse.FormField(
                "days", "请假天数", "NUMBER", true, null, null, "请输入天数", 4),
            new FormConfigResponse.FormField(
                "reason", "请假原因", "TEXTAREA", true, null, null, "请输入请假原因", 5)),
        Arrays.asList(
  ...
  private FormConfigResponse buildOvertimeConfig() {
    return new FormConfigResponse(
        ...
        Arrays.asList(
            ...
            new FormConfigResponse.FormField(
                "endTime", "结束时间", "TIME", true, null, null, "请选择结束时间", 3),
            new FormConfigResponse.FormField(
                "hours", "加班时长", "NUMBER", true, null, null, "请输入小时数", 4),
            new FormConfigResponse.FormField(
                "reason", "加班原因", "TEXTAREA", true, null, null, "请输入加班原因", 5)),
        Arrays.asList(
            new FormConfigResponse.FormAction(
                "submit",
                "提交申请",
