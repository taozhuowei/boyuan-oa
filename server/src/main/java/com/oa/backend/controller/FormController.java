...
public class FormController {
  ...
  /** 获取待审批列表 权限：所有登录角色均可调用，各角色只见自己关联的待办项（HR/EMPLOYEE/WORKER 查看本人提交的待审单据） */
  @GetMapping("/todo")
  @PreAuthorize(
      "hasAnyRole('PROJECT_MANAGER','CEO','FINANCE','DEPARTMENT_MANAGER','HR','EMPLOYEE','WORKER')")
  ...
  /** 获取历史记录 权限：所有角色 CEO/Finance: 查看所有 其他: 只能查看自己的 */
  @GetMapping("/history")
  @PreAuthorize(
  ...
  public ResponseEntity<List<FormRecordResponse>> getHistory(
      Authentication authentication, @RequestParam(required = false) List<String> formTypes) {
    Long currentEmployeeId = getCurrentEmployeeId(authentication);
    if (currentEmployeeId == null) {
      return ResponseEntity.badRequest().build();
    }
    ...
    // 默认查询请假、加班和报销类型
    List<String> types =
        formTypes != null && !formTypes.isEmpty()
            ? formTypes
