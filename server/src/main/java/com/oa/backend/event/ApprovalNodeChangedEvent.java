 ...
 * 审批节点变更事件。
 *
 * <p>当表单审批流程进入新节点时发布此事件，用于通知相关审批人。 事件包含表单记录 ID、新节点名称和需要接收通知的用户 ID 列表。
 *
...
public class ApprovalNodeChangedEvent extends ApplicationEvent {
   ...
   */
  public ApprovalNodeChangedEvent(
      Object source, Long formRecordId, String newNodeName, List<Long> recipientIds) {
    super(source);
