 ...
 * 认证辅助数据服务
 *
 * <p>职责：为 /auth/login 和 /auth/me 响应提供辅助字段查询： - 按 roleCode 查询角色中文名称（role 表） - 按 departmentId
 * 查询部门名称（department 表） - 按 employeeId 查询有效的第二角色编码列表（second_role_assignment 表）
 *
 * <p>所有方法均采用"失败兜底"策略：查询异常时返回安全默认值，不阻塞登录主流程。
 *
 * <p>注意：本服务不依赖 AuthController，不存在循环依赖风险。 调用方：AuthController。
 */
@Slf4j
@Service
...
public class AuthDataService {
  ...
  public String resolveRoleName(String roleCode, String fallback, Object logContext) {
    try {
      ...
      log.warn(
          "AuthData: failed to load role name for context={}, roleCode={}",
          logContext,
          roleCode,
          e);
    }
  ...
  public String resolveDepartmentName(Long departmentId, Object logContext) {
    ...
    try {
      ...
      log.warn(
          "AuthData: failed to load department name for context={}, departmentId={}",
          logContext,
          departmentId,
          e);
    }
  ...
  /**
   * 查询员工有效的第二角色编码列表（revoked=false、deleted=0）。 查询失败兜底返回空列表，不阻塞登录主流程。
   *
  ...
  public List<String> resolveSecondRoles(Long employeeId) {
    try {
      return secondRoleAssignmentMapper
          .selectList(
          ...
          .stream()
          .map(SecondRoleAssignment::getRoleCode)
          .toList();
    } catch (Exception e) {
