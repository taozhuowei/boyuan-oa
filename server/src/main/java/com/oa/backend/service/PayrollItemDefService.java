...
/** 工资项定义服务。 职责：管理系统内置与自定义工资项定义（CRUD）。 系统内置项（isSystem=true）code/type 受保护，不可删除。 */
@Service
...
public class PayrollItemDefService {
  ...
  /**
   * 更新工资项定义。 系统内置项（isSystem=true）的 code/type 受保护（不在此更新，调用方应先校验）。
   *
   * @param id 工资项定义 ID
  ...
  /**
   * 软删除工资项定义。 系统内置项不可删除，调用方应在调用前校验 isSystem。
   *
   * @param id 工资项定义 ID
