 ...
 *   - PROJECT_MANAGER 仅当该附件关联业务记录的 project_id 属于自己作为 PM 管理的项目
 *   - DEPARTMENT_MANAGER 仅当业务记录提交人与自己同部门
 *   - 其它情况拒绝
 *
 * 业务类型（businessType）→ 归属解析：
 *   LOG / INJURY / EXPENSE / LEAVE / OVERTIME 全部经由 form_record 表，
 *   businessId = form_record.id；从 form_record 取 project_id + submitter_id
 *   GENERAL 或未识别类型：无归属信息，仅上传者/审计角色放行
 */
@Service
...
public class AttachmentAccessService {
    ...
    public boolean canAccess(AttachmentMeta meta, Authentication auth, Long currentEmployeeId) {
        ...
        if (authorities.contains("ROLE_PROJECT_MANAGER") && ownership.projectId() != null
        ...
        if (authorities.contains("ROLE_DEPARTMENT_MANAGER") && currentEmployeeId != null
    ...
    /**
     * 根据业务类型和业务 ID 解析归属记录
     * 当前所有 5 种 businessType 均映射到 form_record 表
     * GENERAL 或无法解析返回 null
     */
