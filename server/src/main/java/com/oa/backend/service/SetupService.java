...
/**
 * 系统初始化服务。
 * 负责系统初始化状态检查、初始化向导业务逻辑、恢复码管理等。
 *
 * <p>核心业务规则：
 * <ul>
 *   <li>系统初始化状态通过 system_config 表的 'initialized' 键管理</li>
 *   <li>CEO 账户创建时生成 32 位恢复码，用于密码重置</li>
 *   <li>恢复码使用 BCrypt 哈希存储，明文仅返回一次</li>
 *   <li>验证恢复码后自动轮换，确保一次性使用</li>
 * </ul>
 */
@Slf4j
@Service
...
public class SetupService {
    ...
    /**
     * 初始化系统。
     * 执行完整的初始化流程：创建 CEO、HR 账户，可选创建 OPS、GM 账户和自定义角色。
     *
     * <p>步骤说明：
     * <ol>
     *   <li>创建 CEO 账户，生成恢复码</li>
     *   <li>创建 HR 账户</li>
     *   <li>可选：创建运营总监账户</li>
     *   <li>可选：创建总经理账户</li>
     *   <li>可选：创建自定义角色</li>
    ...
    /**
     * 开发环境重置初始化状态。
     * 仅在开发环境使用，将 initialized 标记为 false。
     */
    ...
    private void createCustomRoles(List<CustomRoleRequest> roles) {
        for (CustomRoleRequest roleReq : roles) {
            if (roleReq.code() == null || roleReq.code().isBlank() ||
                roleReq.name() == null || roleReq.name().isBlank()) {
                continue;
            }
