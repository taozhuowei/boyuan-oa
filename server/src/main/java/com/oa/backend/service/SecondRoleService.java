 ...
 * 第二角色服务
 *
 * <p>职责：管理第二角色定义的查询，以及第二角色分配（SecondRoleAssignment） 的创建与撤销，含业务规则校验和 CEO 通知。
 *
 * <p>业务规则： - 同一 (employee, roleCode, projectId) 不能重复有效分配 - project_bound=true 的角色必须绑定项目 - 角色
 * applies_to 限制员工类型时须匹配 - 分配后通知 CEO；撤销无通知
 *
 * <p>数据来源：second_role_def、second_role_assignment、employee 表。 调用方：SecondRoleController。
 */
@Slf4j
@Service
...
public class SecondRoleService {
  ...
  public List<SecondRoleAssignment> listAssignments(Long employeeId, Long projectId) {
    LambdaQueryWrapper<SecondRoleAssignment> q =
  ...
  /**
   * 创建第二角色分配。 校验：roleCode 存在、project_bound 时 projectId 必填、员工存在、类型匹配、无重复有效分配。 成功后异步通知所有
   * CEO（失败不回滚主事务）。
   *
  ...
  public AssignResult assign(
      Long employeeId, String roleCode, Long projectId, String note, Authentication auth) {
    ...
    if (def.getAppliesTo() != null && !def.getAppliesTo().equals(target.getEmployeeType())) {
      return AssignResult.invalid(
          "员工类别 [" + target.getEmployeeType() + "] 与角色适用范围 [" + def.getAppliesTo() + "] 不符");
    }
    ...
    try {
      ...
      for (Employee c : ceos) {
        notificationService.send(
            ...
            String.format(
                "%s 被分配 %s（%s）",
                target.getName(), def.getName(), projectId != null ? "项目 #" + projectId : "全局"),
            "SYSTEM",
            "SECOND_ROLE",
            a.getId());
      }
  ...
  /** assign 操作结果。 */
  public sealed interface AssignResult permits AssignResult.Ok, AssignResult.Invalid {

    static AssignResult ok(SecondRoleAssignment assignment) {
      return new Ok(assignment);
    }

    static AssignResult invalid(String message) {
  ...
  /** revoke 操作结果。 */
  public sealed interface RevokeResult permits RevokeResult.Ok, RevokeResult.NotFound {

    static RevokeResult ok(Long id) {
      return new Ok(id);
    }

    static RevokeResult notFound() {
      return new NotFound();
    }
