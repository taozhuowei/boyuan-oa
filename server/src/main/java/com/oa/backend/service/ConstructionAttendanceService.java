 ...
 *
 * <p>写入入口： - WorkLogController.submitLog 调用 {@link #recordFromLogForm} 自动写入 - 重新提交日志时先 {@link
 * #softDeleteByForm} 再写入
 *
 * <p>查询入口： - {@link #countDaysByEmployee} 单员工指定项目+周期天数 - {@link #aggregatePerEmployee} 项目周期内全员明细
 *
 * <p>form_data 标准约定（设计 §8.3 工长字段）： { "date": "YYYY-MM-DD", "attendees": [employeeId, ...],
 * "content": "...", "materials": [...] }
 */
@Slf4j
@Service
...
public class ConstructionAttendanceService {
  ...
  /**
   * 从 LOG/INJURY/OVERTIME form_record 中抽取 attendees + date 写入 attendance 表。 source 由调用方传入。幂等：相同
   * (employee_id, date, source_form_id) 触发 UNIQUE 约束 → 静默忽略。
   */
  @Transactional
  public int recordFromLogForm(FormRecord form, Long projectId, String source) {
    ...
    try {
      ...
      try {
        ...
        log.warn("date 格式无效: {} formId={}", dateStr, form.getId());
        return 0;
      }
      ...
      if (att instanceof List<?> list) {
        for (Object o : list) {
          if (o instanceof Number n) empIds.add(n.longValue());
            ...
            try {
              empIds.add(Long.parseLong(o.toString()));
            } catch (NumberFormatException e) {
              log.debug(
                  "attendees item is not a valid numeric id, skipped: formId={}, value={}",
                  form.getId(),
                  o);
            }
          }
  ...
  public void softDeleteByForm(Long formId) {
    ...
    List<ConstructionAttendance> rows =
