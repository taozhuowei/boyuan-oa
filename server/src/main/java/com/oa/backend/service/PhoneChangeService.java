...
/**
 * 手机号变更流程的内存态存储与清理。 从 AuthController 中抽离，遵守"Controller 只做 HTTP 分发，Service 承载业务状态与定时任务"的分层原则。
 *
 * <p>存储三类条目： - currentCodeStore：发送到用户当前手机号的验证码（用于验证所有权） - newCodeStore：发送到用户新手机号的验证码（用于验证新号码持有） -
 * tokenStore：变更令牌（验证完当前手机号后签发，凭此请求新号验证码）
 *
 * <p>所有条目均在写入时附带 expireAt；业务路径每次读取会调用 isExpired() 校验； 本类通过 @Scheduled 周期性回收过期条目以防内存泄漏。
 */
@Service
public class PhoneChangeService {
  ...
  /** 失效给定用户的所有变更令牌。 场景：手机号变更确认成功后，清空该用户名下所有未使用的令牌。 */
  public void removeTokenByUserId(Long userId) {
    tokenStore.entrySet().removeIf(e -> e.getValue().userId().equals(userId));
  }

  /** 每 10 分钟清理一次过期条目，防止 Map 长期运行内存泄漏。 业务路径已在每次访问时 isExpired() 校验，这里仅回收存储空间。 */
