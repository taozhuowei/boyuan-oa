...
/** 补贴三级覆盖解析服务。 解析顺序：EMPLOYEE > POSITION > GLOBAL。上层未命中则回落下一层，全部未命中则视为无该项补贴。 */
@Service
...
public class AllowanceResolutionService {
  ...
  public List<Resolved> resolveForEmployee(Employee employee) {
    List<AllowanceDef> defs =
        allowanceDefMapper.selectList(
  ...
  public BigDecimal resolveAmount(Long allowanceDefId, Long employeeId, Long positionId) {
    AllowanceConfig emp =
        employeeId == null
            ? null
            : allowanceConfigMapper.selectOne(
