 ...
 * 工资条服务
 *
 * 职责：工资条（payroll_slip）的查询、状态变更（DISPUTED），
 * 以及工资条明细（payroll_slip_item）与项目定义（payroll_item_def）的读取与富化。
 *
 * 注意：工资条的确认（电子签名）由 {@link SignatureService} 负责；
 * 本服务仅处理 PayrollController 中直接操作三个 Mapper 的剩余逻辑。
 *
 * 数据来源：payroll_slip、payroll_slip_item、payroll_item_def、employee 表。
...
public class PayrollSlipService {
    ...
    /**
     * 将工资条状态置为 DISPUTED（员工提出异议）。
     * 前置条件：工资条必须处于 PUBLISHED 状态。
     *
    ...
    /**
     * 获取属于当前登录员工的工资条，不属于则返回 null。
     * 用于 confirm/dispute 前的权限校验。
     *
     * @param slipId         工资条 ID
