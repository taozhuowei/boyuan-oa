...
/**
 * 工作台业务逻辑。 聚合个人资料、菜单配置、工作台摘要三类查询，封装多 Mapper 协同。 个人资料查询通过 UserProfileService 完成，享受 Caffeine 60s
 * 缓存，减少重复 DB 访问。
 */
@Service
...
public class WorkbenchService {
  ...
  public WorkbenchSummaryResponse buildWorkbenchSummary(Authentication authentication) {
    ...
    String payrollStatus =
        SecurityUtils.hasFinanceAccess(authentication) ? getLatestPayrollCycleStatus() : null;
    Integer activeProjectCount =
        ("ceo".equals(role) || "project_manager".equals(role)) ? countActiveProjects() : null;
    Integer retentionAlertCount = "ceo".equals(role) ? countPendingRetentionReminders() : null;
  ...
  /**
   * 构建当前登录用户的个人资料。 姓名、部门、员工类型、账号状态均来源于 employee / department 表（经由 UserProfileService 缓存层）， 不依赖任何硬编码
   * demo 账号映射；确保生产环境同名用户也得到真实数据。 缓存策略：UserProfileService.loadByEmployeeNo 带 60s TTL，减少频繁工作台轮询时的 DB
   * 压力。
   */
  public UserProfileResponse buildUserProfile(Authentication authentication) {
    ...
    String displayName =
        (employee.getName() != null && !employee.getName().isBlank())
            ...
            : username;
    String employeeType =
        employee.getEmployeeType() != null ? employee.getEmployeeType() : "OFFICE";
    String accountStatus =
        employee.getAccountStatus() != null ? employee.getAccountStatus() : "ACTIVE";
  ...
  public WorkbenchConfigResponse buildWorkbenchConfig(Authentication authentication) {
    ...
    List<WorkbenchConfigResponse.QuickAction> quickActions =
        Arrays.asList(
            new WorkbenchConfigResponse.QuickAction(
                "leave", "请假", "calendar", "/forms?type=leave", true),
            new WorkbenchConfigResponse.QuickAction(
                "overtime", "加班", "clock-circle", "/forms?type=overtime", true),
            new WorkbenchConfigResponse.QuickAction(
                "notification", "通知", "bell", "/notifications", true));
    WorkbenchConfigResponse.DashboardWidgets widgets =
        new WorkbenchConfigResponse.DashboardWidgets(true, true, true, true);
  ...
  private String getLatestPayrollCycleStatus() {
    List<PayrollCycle> cycles =
        payrollCycleMapper.selectList(
  ...
  /**
   * 按角色构建侧边栏菜单。 单一事实源（SOT）为前端 app/h5/layouts/default.vue 的 ROLE_MENUS；
   * 本方法产物必须与之一一对应（code/label/path 全部一致）， 以保证前端在 API 菜单生效前（首屏静态 fallback）与生效后的体验一致。 路径均为 snake_case
   * 以匹配 Phase A 页面目录重命名结果。
   */
    ...
    return switch (role) {
      case "ceo" ->
          Arrays.asList(
              ...
              new WorkbenchConfigResponse.MenuItem(
                  "todo", "审批中心", "check-circle", "/todo", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "employees", "员工管理", "team", "/employees", true, null),
              new WorkbenchConfigResponse.MenuItem("org", "组织架构", "apartment", "/org", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "positions", "岗位管理", "idcard", "/positions", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "allowances", "补贴配置", "idcard", "/allowances", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "leave_types", "假期配额", "calendar", "/leave_types", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "role", "角色管理", "user-switch", "/role", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "projects", "项目管理", "project", "/projects", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "payroll", "薪资管理", "money", "/payroll", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "construction_log", "施工日志", "file-text", "/construction_log", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "injury", "工伤补偿", "alert", "/injury", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "expense-apply", "费用报销", "wallet", "/expense/apply", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "expense-records", "报销记录", "solution", "/expense/records", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "retention", "数据保留", "safety", "/retention", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "data_export", "数据导出", "export", "/data_export", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "data_viewer", "数据查看", "database", "/data_viewer", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "operation_logs", "操作日志", "audit", "/operation_logs", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "config", "系统配置", "setting", "/config", true, null));
      case "finance" ->
          Arrays.asList(
              new WorkbenchConfigResponse.MenuItem(
                  "workbench", "工作台", "dashboard", "/", true, null),
              ...
              new WorkbenchConfigResponse.MenuItem(
                  "payroll", "薪资管理", "money", "/payroll", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "injury", "工伤理赔", "alert", "/injury", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "expense-apply", "费用报销", "wallet", "/expense/apply", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "expense-records", "报销记录", "solution", "/expense/records", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "allowances", "补贴配置", "idcard", "/allowances", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "positions", "岗位薪资配置", "idcard", "/positions", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "projects", "项目管理", "project", "/projects", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "directory", "通讯录导入", "import", "/directory", true, null));
      case "project_manager" ->
          Arrays.asList(
              ...
              new WorkbenchConfigResponse.MenuItem(
                  "construction_log", "施工日志", "file-text", "/construction_log", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "templates", "工作项模板", "form", "/construction_log/templates", true, null),
      ...
      case "hr" ->
          Arrays.asList(
              ...
              new WorkbenchConfigResponse.MenuItem(
                  "leave_types", "假期配额", "calendar", "/leave_types", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "attendance", "考勤管理", "calendar", "/attendance", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "expense-apply", "费用报销", "wallet", "/expense/apply", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "expense-records", "报销记录", "solution", "/expense/records", true, null));
      case "department_manager" ->
          Arrays.asList(
              ...
              new WorkbenchConfigResponse.MenuItem(
                  "attendance", "考勤管理", "calendar", "/attendance", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "employees", "员工管理", "user", "/employees", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "expense-apply", "费用报销", "wallet", "/expense/apply", true, null));
      case "worker" ->
          Arrays.asList(
              new WorkbenchConfigResponse.MenuItem(
                  "workbench", "工作台", "dashboard", "/", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "attendance", "考勤申请", "calendar", "/attendance", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "construction_log", "施工日志", "file-text", "/construction_log", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "injury", "工伤补偿", "alert", "/injury", true, null),
              new WorkbenchConfigResponse.MenuItem("forms", "表单中心", "file", "/forms", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "payroll", "工资条", "money", "/payroll", true, null),
              new WorkbenchConfigResponse.MenuItem(
                  "expense-apply", "费用报销", "wallet", "/expense/apply", true, null));
