 ...
 * 薪资更正服务。
 *
 * <p>流程： 1. Finance 调用 createCorrection() → 写 payroll_adjustment (status=PENDING)， 并通过
 * ApprovalFlowService 启动 PAYROLL_CORRECTION 审批流； 2. CEO 在 /todo 审批通过 → form_record.status=APPROVED；
 * 3. 后续 syncFromApprovalForms() 检测到关联 form 已 APPROVED 但 applied=false 的调整， 将原 slip 标记
 * SUPERSEDED，按更正项 + 原始项创建新 slip（version+1，status=PUBLISHED）； 4. CEO 驳回 → status=REJECTED，原 slip 不变。
 */
@Slf4j
@Service
...
public class PayrollCorrectionService {
   ...
   */
  @Transactional
  public PayrollAdjustment createCorrection(
      Long slipId, String reason, List<CorrectionItem> corrections, Long financeId) {
  ...
  /**
   * 同步所有"已审批通过但未应用"的更正：将原 slip 置 SUPERSEDED，生成 version+1 的新 slip。 由 GET /payroll/corrections
   * 等读路径触发，确保眼下数据反映最新审批状态。
   */
  @Transactional
  public int syncFromApprovalForms() {
    List<PayrollAdjustment> pending =
        adjustmentMapper.selectList(
  ...
  /**
   * 实际应用更正：原 slip → SUPERSEDED；新 slip 写入 PUBLISHED，version+1。 新 slip 的明细在原明细基础上按 corrections 替换金额（按
   * item_def_id 匹配）。
   */
  @Transactional
  public void applyCorrection(PayrollAdjustment adj) {
    ...
    try {
      corrections =
          objectMapper.readValue(
              adj.getCorrectionsJson(), new TypeReference<List<CorrectionItem>>() {});
    } catch (Exception e) {
    ...
    log.info(
        "薪资更正已应用: adjId={}, oldSlipId={}, newSlipId={}, version={}",
        adj.getId(),
        oldSlip.getId(),
        newSlip.getId(),
        newSlip.getVersion());
  ...
  public List<PayrollAdjustment> list(Long cycleId, Long employeeId) {
    ...
    LambdaQueryWrapper<PayrollAdjustment> q =
        new LambdaQueryWrapper<PayrollAdjustment>()
            .eq(PayrollAdjustment::getDeleted, 0)
