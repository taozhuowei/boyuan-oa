...
/** 部门管理服务 职责：封装部门（Department）及员工人数统计的持久层操作， 供 DepartmentController 调用，隔离控制器与 Mapper 细节。 */
@Service
...
public class DepartmentService {
  ...
  /**
   * 统计每个部门的在职员工数（状态为 ACTIVE 且未软删除）。 key = department_id，value = 员工数。
   *
  ...
  public boolean hasActiveEmployees(Long departmentId) {
    ...
    wrapper.eq("department_id", departmentId).eq("deleted", 0).eq("account_status", "ACTIVE");
