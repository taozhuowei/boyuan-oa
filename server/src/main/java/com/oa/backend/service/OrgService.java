 ...
 * 组织架构服务
 *
 * <p>职责： - 构建组织架构树（员工节点 + 部门/岗位/角色名称映射） - 更新员工直系领导，含循环汇报校验 - 构建单个员工的 OrgNodeResponse（备用路径）
 *
 * <p>数据来源：employee、department、position、role 四张表。 调用方：OrgController（CEO 专属接口）。
 */
@Slf4j
@Service
...
public class OrgService {
  ...
  /**
   * 构建完整组织架构树。 辅助字段（部门名、岗位名、角色名）加载失败时兜底为空值，不阻塞主流程。
   *
  ...
  /**
   * 更新员工直系领导。 校验：员工存在、新上级存在、不能自己设自己、不能将下属设为上级。
   *
  ...
  /**
   * 构建单个员工的节点响应（备用路径，逐字段查询）。 各辅助字段查询失败时兜底为空，不阻塞响应构建。
   *
   * @param emp 员工实体
  ...
  public OrgNodeResponse buildSimpleNodeResponse(Employee emp) {
    ...
    try {
      ...
      log.warn(
          "OrgNode: failed to load department for empId={}, departmentId={}",
          emp.getId(),
          emp.getDepartmentId(),
          e);
    }
    ...
    try {
      ...
      log.warn(
          "OrgNode: failed to load position for empId={}, positionId={}",
          emp.getId(),
          emp.getPositionId(),
          e);
    }
    ...
    try {
      Role role = roleMapper.selectOne(new QueryWrapper<Role>().eq("role_code", emp.getRoleCode()));
      if (role != null) roleName = role.getRoleName();
    } catch (Exception e) {
      log.warn(
          "OrgNode: failed to load role for empId={}, roleCode={}",
          emp.getId(),
          emp.getRoleCode(),
          e);
    }
