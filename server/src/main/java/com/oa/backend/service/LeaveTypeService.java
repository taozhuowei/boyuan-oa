...
/** 请假类型服务 职责：封装请假类型定义（LeaveTypeDef）的增删改查逻辑， 供 LeaveTypeController 调用，隔离控制器与持久层细节。 */
@Service
...
public class LeaveTypeService {
  ...
  /**
   * 查询启用中的请假类型列表（用于考勤表单下拉框）。 结果按 displayOrder 升序排列，已软删除的记录不返回。
   *
   * @return 启用状态的请假类型列表
   */
  public List<LeaveTypeDef> listEnabledLeaveTypes() {
    ...
    wrapper
        .eq(LeaveTypeDef::getIsEnabled, true)
  ...
  /**
   * 查询全部请假类型（含禁用项），供 HR 管理界面使用。 结果按 displayOrder 升序排列，已软删除的记录不返回。
   *
   * @return 所有未删除的请假类型列表
   */
  public List<LeaveTypeDef> listAllLeaveTypes() {
    LambdaQueryWrapper<LeaveTypeDef> wrapper = new LambdaQueryWrapper<>();
    wrapper.eq(LeaveTypeDef::getDeleted, 0).orderByAsc(LeaveTypeDef::getDisplayOrder);
    return leaveTypeDefMapper.selectList(wrapper);
  }
  ...
  /**
   * 创建新请假类型。 isSystem 固定为 false，isEnabled 固定为 true，displayOrder 自动追加到末尾。
   *
   * @param code 请假类型编码
   * @param name 请假类型名称
   ...
   * @return 已持久化的请假类型实体
   */
  public LeaveTypeDef createLeaveType(
      String code,
      String name,
      Integer quotaDays,
   ...
   * @return 更新后的实体
   */
  public LeaveTypeDef updateLeaveType(
      LeaveTypeDef entity,
      String name,
      Integer quotaDays,
      BigDecimal deductionRate,
  ...
  private Integer nextDisplayOrder() {
    ...
    return (last != null && last.getDisplayOrder() != null) ? last.getDisplayOrder() + 1 : 1;
  }
}
