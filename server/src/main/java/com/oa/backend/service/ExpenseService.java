...
public class ExpenseService {
  ...
  public FormRecordResponse submitExpense(Long submitterId, ExpenseSubmitRequest request) {
    ...
    log.info(
        "报销申请提交成功: claimId={}, formId={}, submitterId={}, amount={}",
        claim.getId(),
        formResponse.id(),
        submitterId,
        request.totalAmount());
  ...
  public ExpenseClaimResponse getExpenseDetail(Long formId, Long requesterId) {
    ...
    List<ExpenseItemDto> itemDtos =
        items.stream().map(this::convertToItemDto).collect(Collectors.toList());
