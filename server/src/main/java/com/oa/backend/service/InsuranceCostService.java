...
/**
 * 保险成本聚合服务（设计 §8.4）。 公式：本期成本 = Σ daily_rate × Σ 适用员工去重出勤天数（从 construction_attendance 真实数据）
 *
 * <p>适用员工解析： - GLOBAL：项目内所有 LABOR 类员工 - POSITION：项目内 position_id = scope_target_id 的所有 LABOR -
 * EMPLOYEE：scope_target_id 这一员工
 *
 * <p>周期由调用方（前端默认本月 1 号 ~ 今天）传入；effective_date 之前的出勤不计入。
 */
@Service
...
public class InsuranceCostService {
  ...
  public ItemCost computeOne(ProjectInsuranceDef def, LocalDate start, LocalDate end) {
    LocalDate effectiveStart =
        def.getEffectiveDate() != null && def.getEffectiveDate().isAfter(start)
    ...
    Map<Long, Long> attendance =
        attendanceService.aggregatePerEmployee(def.getProjectId(), effectiveStart, end);
    long totalDays = 0;
    for (Long empId : applicable) {
      totalDays += attendance.getOrDefault(empId, 0L);
    }
    BigDecimal cost =
        def.getDailyRate() == null
            ? BigDecimal.ZERO
