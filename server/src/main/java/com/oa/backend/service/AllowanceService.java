...
/**
 * 补贴项（allowance_def）及三级覆盖配置（allowance_config）CRUD 服务。 负责补贴定义的创建、更新、软删除以及配置项的批量替换。 补贴解析逻辑（三级优先级查找）由
 * AllowanceResolutionService 负责。
 */
@Service
...
public class AllowanceService {
   ...
   */
  @Transactional
  public AllowanceDef updateDef(
      Long id, String name, String description, Integer displayOrder, Boolean isEnabled) {
  ...
  /**
   * 批量替换指定补贴项的全部配置项（整表替换策略）。 先物理删除旧配置，再插入新配置，保持一致性。
   *
