 ...
 * 用户档案查询服务（可缓存层）。
 *
 * <p>职责：封装 employee + department 的联合查询，并通过 @Cacheable 在 Spring AOP 代理上缓存结果。 数据来源：employee 表（按
...
public class UserProfileService {
  ...
  /** 员工档案值对象，聚合 employee 实体与已解析的部门名称。 不暴露为 public 外部使用，故设为包私有；调用方仅通过 loadByEmployeeNo 获取。 */
