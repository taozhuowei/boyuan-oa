...
/**
 * 项目营收业务服务（设计 §8.5 营收管理）。
 * 封装按项目查询里程碑（用于收款展示）、更新收款字段、以及营收汇总计算。
 * 合同金额变更走跨方审批，见 {@link RevenueChangeService}。
 */
@Service
...
public class ProjectRevenueService {
    ...
    /**
     * 更新里程碑的收款字段（receipt_status / actual_receipt_amount / receipt_date / receipt_remark），
     * 刷新 updated_at 时间戳。
     * 注意：contract_amount 不在此修改，须通过 {@link RevenueChangeService#createChange} 走跨方审批。
     *
