...
public class ApprovalFlowService {
  ...
  /**
   * 全量替换指定审批流的节点配置。 策略：软删除旧节点，插入新节点列表。
   *
   * @param flowId 审批流定义 ID
   * @param nodeSpecs 新节点规格列表，每项包含 nodeName/approverType/approverRef/skipCondition
   */
  @Transactional
  public List<ApprovalFlowNode> replaceFlowNodes(
      Long flowId, List<ApprovalFlowNodeSpec> nodeSpecs) {
    // 软删除现有节点
  ...
  public record ApprovalFlowNodeSpec(
      String nodeName, String approverType, String approverRef, String skipCondition) {}

  /** 初始化审批流 加载激活的流程定义，评估 skipCondition，设置第一个有效节点，设置表单状态为 PENDING */
  @Transactional
  public void initFlow(Long formRecordId, String formType, Long submitterId) {
    ...
    log.info(
        "审批流初始化完成: formId={}, formType={}, startNodeOrder={}",
        formRecordId,
        formType,
        startNode != null ? startNode.getNodeOrder() : "ALL_SKIPPED");
  }

  /**
   * 评估节点跳过条件 格式：JSON {"role":"project_manager"} → 提交人角色匹配时跳过
   *
  ...
  public boolean canApprove(Long formRecordId, Long employeeId) {
    ...
    // 查找审批流定义
    ApprovalFlowDef flowDef =
        approvalFlowDefMapper.findActiveByBusinessType(formRecord.getFormType());
    if (flowDef == null) {
      return false;
    }

    // 获取当前节点
    ApprovalFlowNode currentNode =
        approvalFlowNodeMapper.findByFlowIdAndNodeOrder(
  ...
  /**
   * 推进审批流 验证权限、插入审批记录、移动到下一节点或完成流程
   *
  ...
  @Transactional
  public FormRecordResponse advance(
      Long formRecordId, Long approverId, String action, String comment) {
    // 获取表单记录
    FormRecord formRecord = formRecordMapper.selectById(formRecordId);
    if (formRecord == null) {
      throw new IllegalStateException("表单记录 [" + formRecordId + "] 不存在");
    ...
    if ("REJECT".equals(action)) {
      ...
      try {
        notificationService.send(
            ...
            "Your "
                ...
                + " request has been rejected. Comment: "
        ...
        log.warn(
            "Failed to send rejection notification for formId={}: {}",
            formRecordId,
            e.getMessage());
      }
      ...
      Optional<ApprovalFlowNode> nextNodeOpt =
          allNodes.stream()
      ...
      if (nextNodeOpt.isPresent()) {
        ...
        try {
          ...
          log.warn(
              "Failed to send approval notification for formId={}: {}",
              formRecordId,
              e.getMessage());
        }
      }
    ...
    log.info(
        "审批流推进完成: formId={}, action={}, newStatus={}",
        formRecordId,
        action,
  ...
  /** 获取审批历史记录 批量查询 approver 信息，避免循环内 selectById 导致 N+1 查询 */
  ...
  private String getNodeNameByOrder(Long formId, Integer nodeOrder) {
    ...
    ApprovalFlowNode node =
        approvalFlowNodeMapper.findByFlowIdAndNodeOrder(flowDef.getId(), nodeOrder);
  ...
  private void writeOperationLog(
      Long operatorId, String action, String targetType, Long targetId, String detail) {
