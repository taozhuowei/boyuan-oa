...
public class FormService {
  ...
  public FormRecordResponse submitForm(
    ...
    log.info(
        "表单提交成功: formId={}, formType={}, submitterId={}",
        formRecord.getId(),
        formType,
        submitterId);
  ...
  /**
   * 获取历史记录 CEO/Finance: 查看所有指定类型的表单 其他角色: 只能查看自己提交的表单
   *
   ...
   * @return 表单记录响应列表
   */
  public List<FormRecordResponse> getHistory(
      Long employeeId, String roleCode, List<String> formTypes) {
    List<FormRecord> records;

    // 判断是否有查看全部权限（CEO、Finance 或 PM）
    if ("ceo".equalsIgnoreCase(roleCode)
        || "finance".equalsIgnoreCase(roleCode)
    ...
    return records.stream().map(this::buildFormRecordResponse).collect(Collectors.toList());
  }
  ...
  /**
   * 获取员工的待审批列表 委托给 ApprovalFlowService
   *
   * @param employeeId 员工 ID
   * @return 表单记录响应列表
   */
