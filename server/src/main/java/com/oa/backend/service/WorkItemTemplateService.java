 ...
 * 工作项模板服务类。
 *
 * <p>负责工作项模板的 CRUD 和派生操作，封装 WorkItemTemplateMapper 和 EmployeeMapper 的调用。 数据来源：work_item_template
 * 表（items 字段以 JSON 数组存储）。
 */
@Service
...
public class WorkItemTemplateService {
  ...
  /**
   * 查询所有未删除的模板，按创建时间降序排列。 如果指定 projectId，则只返回该项目的模板。
   *
   * @param projectId 项目 ID（可为 null，表示查询全部）
   * @return 模板列表
   */
  public List<WorkItemTemplate> listTemplatesByProjectId(Long projectId) {
    LambdaQueryWrapper<WorkItemTemplate> qw =
        new LambdaQueryWrapper<WorkItemTemplate>()
