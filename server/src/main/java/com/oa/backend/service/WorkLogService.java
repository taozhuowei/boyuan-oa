 ...
 * 施工日志服务类。
 *
 * <p>负责施工日志和工伤申报中直接操作数据库的辅助逻辑，封装 FormRecordMapper、 ProjectMapper 和 EmployeeMapper 的调用。
 *
 * <p>注意：审批流程逻辑委托给 FormService 和 ApprovalFlowService； 出勤记录委托给 ConstructionAttendanceService； 站内通知委托给
 * NotificationService。 本服务只处理上述服务未覆盖的直接 Mapper 操作。
 */
@Service
