 ...
 *
 * <p>Intercepts all methods annotated with @OperationLogRecord and writes a log entry to
 * operation_log after successful execution. On failure, the exception is re-thrown without logging
 * (failed operations are not considered completed).
 *
 * <p>The log entry captures: - operatorId: resolved from the current SecurityContext principal -
 * action / targetType: from the annotation attributes - detail: JSON snippet containing method name
 * and return value class name - actedAt: current timestamp
 */
@Slf4j
@Aspect
@Component
...
public class OperationLogAspect {
  ...
  public Object logOperation(ProceedingJoinPoint joinPoint, OperationLogRecord logRecord)
      throws Throwable {
    ...
    try {
      ...
      writeLog(operatorId, logRecord.action(), logRecord.targetType(), buildDetail(joinPoint));
    } catch (Exception e) {
      // Log-write failure must never break the business operation
      log.warn(
          "OperationLogAspect: failed to write operation log for action={}", logRecord.action(), e);
    }
