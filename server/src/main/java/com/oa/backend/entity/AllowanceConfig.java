...
/**
 * 补贴三级覆盖配置实体，对应 allowance_config 表。
 * scope: GLOBAL（scope_target_id 为 NULL）/ POSITION（scope_target_id 为 position.id）/ EMPLOYEE（scope_target_id 为 employee.id）。
 * 同一 (def_id, scope, target_id) 唯一有效；结算时按 EMPLOYEE > POSITION > GLOBAL 解析。
 */
@Data
